...
<a href="#" class="project-row group relative z-10 hover:z-50 flex flex-col md:grid md:grid-cols-12 md:items-end pt-8 md:pt-14 pb-8 md:pb-3 border-b border-gray-200 hover:bg-gray-50/50 transition-colors px-0 md:px-4 -mx-0 md:-mx-4 cursor-pointer gap-2 md:gap-0">
    <img src="{{ asset($work['image']) }}" alt="{{ $work['title'] }}" class="block md:hidden w-full aspect-[4/3] object-cover rounded-sm mb-4 shadow-sm">
    ...
    <img src="{{ asset($work['image']) }}" alt="{{ $work['title'] }}" class="hover-image fixed top-0 left-0 hidden md:block w-72 h-48 md:w-[320px] md:h-[220px] object-cover rounded-xl shadow-2xl pointer-events-none opacity-0 scale-95 rotate-0 transition-[opacity,transform] duration-500 ease-out group-hover:opacity-100 group-hover:scale-105 group-hover:-rotate-2 z-50">
</a>